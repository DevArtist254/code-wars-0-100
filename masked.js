...
function maskify(cc) {
  let stringed = String(cc);

  if (stringed.length > 4) {
    const sec = stringed.split('');
    const lastForDigits = sec.splice(-4, 4);
    const hashedValue = sec.map((el) => '#');

    const hashedString = [...hashedValue, ...lastForDigits].join('');
    return hashedString;
  }
